...
namespace PreySense.UI
{
    ...
    public class Slider : Control
    {
        ...
        // Multiplier applied to the thumb radius (1 = default). Lower = smaller thumb.
        private float _thumbScale = 1f;
        public float ThumbScale
        {
            get => _thumbScale;
            set { _thumbScale = value; Invalidate(); }
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            // Draw empty track as a rounded line
            using (Pen penEmpty = new Pen(brushEmpty, _barSize.Height))
            {
                penEmpty.StartCap = LineCap.Round;
                penEmpty.EndCap = LineCap.Round;
                e.Graphics.DrawLine(penEmpty, _barPos.X, _barPos.Y + _barSize.Height / 2, _barPos.X + _barSize.Width, _barPos.Y + _barSize.Height / 2);
            }

            // Draw accent track as a rounded line
            if (thumbX > _barPos.X)
            {
                using (Pen penAccent = new Pen(brushAccent, _barSize.Height))
                {
                    penAccent.StartCap = LineCap.Round;
                    penAccent.EndCap = LineCap.Round;
                    e.Graphics.DrawLine(penAccent, _barPos.X, _barPos.Y + _barSize.Height / 2, thumbX, _barPos.Y + _barSize.Height / 2);
                }
            }
            ...
            // Draw thumb as a solid circle of accentColor with a dark outline
            float thumbRadius = _radius * 0.5f * _thumbScale * (_innerScale / InnerNormal);
            e.Graphics.FillCircle(brushAccent, thumbX, _thumbPos.Y, thumbRadius);
            using (Pen penBorder = new Pen(Color.FromArgb(255, 64, 64, 64), 1f))
            {
                e.Graphics.DrawCircle(penBorder, thumbX, _thumbPos.Y, thumbRadius);
            }
        }
