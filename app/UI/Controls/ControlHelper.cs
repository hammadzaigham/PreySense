...
public static class ControlHelper
{
    ...
    /// <summary>
    /// Applies the dark/light Explorer visual style to a control's native handle.
    /// Needed for checkboxes created in code after the initial theming pass, whose
    /// glyph would otherwise render with the default (white) Win32 theme.
    /// </summary>
    public static void ApplyExplorerTheme(Control control, bool darkMode)
    {
        if (control == null) return;
        SetWindowTheme(control.Handle, darkMode ? "DarkMode_Explorer" : "Explorer", null);
    }
    ...
    private static void AdjustControls(Control.ControlCollection controls)
    {
        foreach (Control control in controls)
        {
            ...
            if (button != null)
            {
                if (button.Name == "buttonBatteryFull" && button.Activated)
                {
                    button.BackColor = RForm.colorStandard;
