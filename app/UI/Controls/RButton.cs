...
namespace PreySense.UI
{
    public class RButton : Button
    {
        ...
        // When painting text-only buttons we draw the caption ourselves (see OnPaint)
        // so it is vertically centered. While that happens we hide the text from the
        // base renderer to avoid a double-drawn, slightly-low caption.
        [AllowNull]
        public override string Text
        {
            get => suppressText ? string.Empty : base.Text;
            set => base.Text = value;
        }
        ...
        {
            // Draw plain text captions ourselves so they are truly vertically
            // centered. Buttons that carry an Image keep the base layout engine,
            // which handles ImageAlign / TextImageRelation positioning.
            bool textOnly = Image is null && !string.IsNullOrEmpty(base.Text);

            suppressText = textOnly;
            base.OnPaint(pevent);
            suppressText = false;
            ...
            using (Pen penSurface = new Pen(Parent?.BackColor ?? Color.FromArgb(28, 28, 28), border))
            {
                ...
                else if (hovered && !Borderless && hoverBorderColor.A > 0)
                {
                    Rectangle borderRect = new Rectangle(border, border, rectSurface.Width - 2 * border, rectSurface.Height - 2 * border);
                    ControlHelper.DrawGradientBorder(pevent.Graphics, borderRect, hoverBorderColor, radius, 1f, PenAlignment.Inset, 0.10f);
                }
            }
            ...
            if (textOnly)
            {
                ...
                TextRenderer.DrawText(pevent.Graphics, base.Text, Font, textRect, textColor, flags);
            }
